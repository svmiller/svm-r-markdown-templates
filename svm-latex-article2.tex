...
\titleformat*{\subsection}{$if(sansitup)$\small\bfseries\sffamily\uppercase$else$\normalsize\itshape$endif$}
...

{
   ...
   $if(sansitup)$
$if(anonymous)$\hfill $else$$for(author)$\MakeUppercase{\textsf{\Large $author.name$}}, \small{$author.affiliation$}  $sep$ \par \vskip -5.5pt $endfor$ $endif$
$else$
$if(anonymous)$\hfill $else$$for(author)$\MakeUppercase{$author.name$}, \small{$author.affiliation$}  $sep$ \par \vskip -5.5pt $endfor$ $endif$
$endif$
